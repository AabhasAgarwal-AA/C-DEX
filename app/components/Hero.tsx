"use client";
import { signIn, useSession } from "next-auth/react";
import { useRouter } from "next/navigation";

export const Hero = () => {
    const session = useSession();
    const router = useRouter();
    
    const handlePrimary = () => {
        if (session.data?.user) router.push("/dashboard");
        else signIn("google");
    };

    return (
        <div className="hero-wrap">
            <div className="circle-accent" />
            <div className="smear-accent" />

            <div className="coin coin-btc">BTC</div>
            <div className="coin coin-eth">ETH</div>
            <div className="coin coin-sol">SOL</div>
            <div className="coin coin-inr">₹</div>

            <div className="hero-inner">
                <div className="hero-badge">
                    <span className="badge-dot" />
                    India's crypto exchange · INR ready
                </div>

                <h1 className="hero-headline">
                    The Indian<br />
                    Cryptocurrency <span className="accent">Revolution</span>
                </h1>

                <p className="hero-sub">
                    Create a frictionless wallet with just your Google Account.
                    Convert INR into crypto in minutes.
                </p>

                <div className="hero-cta-row">
                    <button className="btn-primary" onClick={handlePrimary}>
                        <svg className="google-icon" viewBox="0 0 24 24" fill="none">
                            <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="rgba(255,255,255,0.9)" />
                            <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="rgba(255,255,255,0.75)" />
                            <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z" fill="rgba(255,255,255,0.6)" />
                            <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="rgba(255,255,255,0.55)" />
                        </svg>
                        {session.data?.user ? "Go to Dashboard" : "Continue with Google"}
                    </button>
                    {/* {!session.data?.user && (
                        <button className="btn-secondary">Learn more ↗</button>
                    )} */}
                </div>

                <div className="stats-row">
                    <div className="stat">
                        <div className="stat-val"><span className="blue">₹0</span> fees</div>
                        <div className="stat-lbl">First swap free</div>
                    </div>
                    <div className="stat">
                        <div className="stat-val"><span className="blue">10+</span> coins</div>
                        <div className="stat-lbl">Supported assets</div>
                    </div>
                    <div className="stat">
                        <div className="stat-val"><span className="blue">2</span> min</div>
                        <div className="stat-lbl">Avg. setup time</div>
                    </div>
                </div>
            </div>
        </div>
    );
};